...
test("report and OCR locale resources keep full English coverage", async () => {
  const reportZh = await loadJson(new URL("../src/lib/report/report-content.json", import.meta.url))
  const reportEn = await loadJson(new URL("../src/lib/report/report-content.en.json", import.meta.url))
  const ocrZh = await loadJson(new URL("../src/lib/ocr/ocr-confirmation.json", import.meta.url))
  const ocrEn = await loadJson(new URL("../src/lib/ocr/ocr-confirmation.en.json", import.meta.url))

  assertSameShape(reportZh, reportEn, "report")
  assertSameShape(ocrZh, ocrEn, "ocr")
})
