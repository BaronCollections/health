...
test("release checklist locale resources keep full English coverage", async () => {
  const zhPath = new URL("../src/lib/release-checklist/mock-content.json", import.meta.url)
  const enPath = new URL("../src/lib/release-checklist/mock-content.en.json", import.meta.url)
  ...
  assert.equal(zhExists, true, "release checklist Chinese resource file should exist")
  assert.equal(enExists, true, "release checklist English resource file should exist")

  const contentZh = await loadJson(zhPath)
  const contentEn = await loadJson(enPath)

  assertSameShape(contentZh, contentEn, "releaseChecklist")
})
