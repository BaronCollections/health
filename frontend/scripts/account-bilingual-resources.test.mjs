...
test("account locale resources keep full English coverage", async () => {
  const zhPath = new URL("../src/lib/account/mock-content.json", import.meta.url)
  const enPath = new URL("../src/lib/account/mock-content.en.json", import.meta.url)
  ...
  assert.equal(zhExists, true, "account Chinese resource file should exist")
  assert.equal(enExists, true, "account English resource file should exist")

  const accountZh = await loadJson(zhPath)
  const accountEn = await loadJson(enPath)

  assertSameShape(accountZh, accountEn, "account")
})
