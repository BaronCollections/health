...
test("OCR upload locale resources keep full English coverage", async () => {
  const uploadZh = await loadJson(new URL("../src/lib/ocr-upload/ocr-upload.json", import.meta.url))
  const uploadEn = await loadJson(new URL("../src/lib/ocr-upload/ocr-upload.en.json", import.meta.url))

  assertSameShape(uploadZh, uploadEn, "ocrUpload")
})
